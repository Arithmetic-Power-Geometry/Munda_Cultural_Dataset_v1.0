...
st.markdown('''<style>.block-container{max-width:1450px;padding-top:1.2rem}.hero{padding:26px;border:1px solid #d9e1d8;border-radius:20px;background:#f7faf5}.hero h1{font-family:Georgia,serif}.small{font-size:.86rem;color:#58645d}[data-testid="stSidebar"]{background:#f1f6ef}.group{font-weight:800;color:#153f2d;margin-top:.6rem}.leader{height:100%;padding:18px;border:1px solid #d9e1d8;border-radius:16px;background:#fff}.leader h3{font-family:Georgia,serif;color:#153f2d;margin-bottom:.2rem}.role{font-weight:800;color:#5b2a1a;margin-bottom:.7rem}.principle{padding:16px;border-left:4px solid #2d6845;background:#f7faf5;border-radius:8px}</style>''',unsafe_allow_html=True)
GROUPS={'Discover':['Home Research Dashboard','Universal Search','Culture Explorer','Life from Birth to Burial'],'Culture & Knowledge':['Language & Lexicon','Kinship & Kili','Festivals & Rituals','Beliefs & Sacred Life','Stories & Oral Traditions','Songs / Dance / Music','Food & Agriculture','Ecology & Ethnobotany','Material Culture & Crafts','Dress & Ornament','Houses & Architecture','Livelihood & Economy','Customary Law & Governance','Education / Health / Demography'],'People & Place':['Places & Landscapes','Geographic / Community Variation'],'History & Change':['Historical Timeline','Historical Archives','Contemporary Change'],'Research Library':['Master Munda Source Census','Mundarica I–XVI Digital Library','Books / Journals / Theses','Government & Archives','Media Archive'],'Evidence & Research':['Evidence Explorer','Contradictions & Variants','Community Validation','Research Gaps / Completeness','Reports & Downloads'],'MLHKP':['About','Governance & Ethics','Contribute / Correct']}
...
def universal_search():
 ...
 q=st.text_input('Search Munda knowledge',placeholder='term, Kili, place, festival, person, source, author…'); c=st.columns(4); typ=c[0].selectbox('Content',['All','source','Mundarica','evidence']); ver=c[1].text_input('Verification filter'); yr=c[2].text_input('Year'); lang=c[3].text_input('Language'); data=source_rows()
 ...
 def hit(x):
  blob=' '.join(str(x.get(k,'')) for k in ['id','title','author','year','domain','geography','language','period','verification']).lower(); return (not q or q.lower() in blob) and (typ=='All' or x['type']==typ) and (not ver or ver.lower() in str(x.get('verification','')).lower()) and (not yr or yr in str(x.get('year',''))) and (not lang or lang.lower() in str(x.get('language','')).lower())
 res=[x for x in data if hit(x)]; st.write(f'**{len(res)} permitted result(s)**')
 for x in res[:200]:
  with st.expander(f"{x['type']} · {x.get('title') or x.get('id')}"):
   st.write('**Permanent ID:**',x.get('id')); st.write('**Domain:**',x.get('domain') or '—'); st.write('**Verification:**',x.get('verification') or '—'); st.write('**Access:**',x.get('access') or '—'); st.write('**Geography:**',x.get('geography') or '—'); s=x['source']
...
def mundarica():
 st.header('Mundarica I–XVI Digital Library'); st.caption('All sixteen permanent slots remain visible. Scan, OCR, transcription, human review and machine completeness are separate layers.'); c=st.columns(5); c[0].metric('Volumes',16); c[1].metric('Located external',a.get('externally_located_volumes',0)); c[2].metric('Page-accounted',a.get('page_accounting_complete_volumes',0)); c[3].metric('Human review','I–XVI' if att.get('attestation',{}).get('status')=='human_review_attested' else '—'); c[4].metric('VERIFIED COMPLETE',a.get('verified_complete_volumes',0)); st.warning('OCR is never treated as verified transcription. Human attestation does not automatically set VERIFIED COMPLETE.')
 summary=[]
 for v in vols:
 ...
 st.dataframe(summary,use_container_width=True,hide_index=True); q=st.text_input('Search this library metadata'); choices=[v for v in vols if not q or q.lower() in json.dumps(v).lower()]; opts=[f"Volume {v['volume']} · {v['source_id']}" for v in choices]
 if not opts:return
 ...
 with tabs[8]:
  st.write('**Human review:**',att.get('attestation',{}).get('status','not recorded'),'· Lead:',att.get('attestation',{}).get('named_lead_reviewer','—')); st.caption(att.get('provenance',{}).get('uncertainty','')); gates=[('Permanent ID',bool(v.get('source_id'))),('Source/artifact located',bool(e) or n==1),('Page accounting',n==1 and bool(a.get('volume_1_page_order_complete'))),('Authoritative artifact reconciled',n==1 and bool(a.get('volume_1_scan_registered'))),('Verified transcription',bool(v.get('verified_transcription_complete'))),('Structured audit',bool(v.get('structured_content_complete'))),('Completeness audit',bool(v.get('verified_complete')))]; st.dataframe([{'Gate':x,'State':'PASS' if y else 'PENDING'} for x,y in gates],hide_index=True,use_container_width=True)
def generic(title,req):
 st.header(title); metrics(); ev=evidence_rows(); hits=[x for x in ev if title.lower().split()[0] in (' '.join(str(v) for v in x.values())).lower()]; st.dataframe(hits,use_container_width=True,hide_index=True) if hits else empty(req)
def leadership():
 st.subheader('Founding leadership & responsibilities'); cols=st.columns(3)
 cards=[('Dr. Mohammad Amir Khusru Akhtar','Founder · Founding Chairperson · Founding Principal Investigator','Principal intellectual creator and scholarly lead of MLHKP/MCD. Leads the research agenda, methodology, dataset architecture, schema, ontology, codebooks, validation standards, research instruments, scholarly resources, books and papers, with final responsibility for scholarly methodology, interpretation and scholarly approval.'),('Dr. Arvind Hans','Founding Project Director','Leads project management, field operations, external review, expert network, media and outreach. Coordinates implementation schedules, primary-data collection under approved instruments, consent/source/field-metadata quality control, external experts, publishers, media operations and approved community programmes.'),('Mr. Rajan Pahan','Founding Community, Meetings & Field Logistics Coordinator','Coordinates community consultations, culturally appropriate introductions, elders, Pahans, customary leaders, practitioners, families, youth, women, performers, artisans, musicians and storytellers; local communication, venues, travel, recording locations, guides, participant mobilisation and follow-up records. This role does not independently determine scholarly interpretation or final scholarly approval.')]
...
if page=='Home Research Dashboard':
...
elif page=='Master Munda Source Census':st.header(page);st.caption('Only repository-calculated census records are displayed; no completeness claim is inferred.');st.dataframe(masters,use_container_width=True,hide_index=True) if masters else empty('canonical source records and systematic search-log entries.')
elif page=='Evidence Explorer':st.header(page);ev=evidence_rows();st.dataframe(ev,use_container_width=True,hide_index=True) if ev else empty('public provenance-linked claims and evidence.')
 ...
 st.header('Governance & Ethics'); st.caption('How MLHKP protects evidence integrity, community interests, cultural access and accountable scholarship.')
 st.markdown('<div class="principle"><b>Governing principle</b><br>Cultural access and consent take precedence over public, institutional or commercial entitlement.</div>',unsafe_allow_html=True)
 t1,t2,t3,t4=st.tabs(['Access & consent','Evidence integrity','Rights & reuse','Accountability'])
 with t1:st.write('Public availability does not automatically make culturally sensitive, sacred, private or community-controlled material appropriate for unrestricted display or reuse. Access decisions remain subject to consent and cultural restrictions.')
 with t2:st.write('MLHKP preserves source, passage/segment/event/object, evidence and claim as distinct layers. OCR is not verified transcription, and historical or source-reported statements are not automatically treated as universal or current facts.')
 with t3:st.write('Third-party works retain their own rights. Repository availability does not establish redistribution or commercial-reuse permission. Project intellectual-property claims do not imply ownership of the Munda community, identity, culture, sacred traditions or collective heritage.')
 with t4:st.write('Corrections, review decisions, provenance, uncertainty and verification state are retained as auditable records. Community validation and scholarly verification are related but distinct processes.')
 leadership(); st.subheader('Decision boundaries'); st.dataframe([{'Area':'Scholarly methodology and interpretation','Responsibility':'Founding PI / scholarly review process'},{'Area':'Project implementation and field operations','Responsibility':'Founding Project Director within approved research/governance procedures'},{'Area':'Community meetings and field logistics','Responsibility':'Founding Community, Meetings & Field Logistics Coordinator; no independent final scholarly-approval authority'},{'Area':'Cultural access / consent','Responsibility':'Applicable consent and cultural-access rules override entitlement'},{'Area':'Machine VERIFIED COMPLETE','Responsibility':'Only after required machine-readable completeness gates pass'}],hide_index=True,use_container_width=True)
elif page=='About':st.header('About MLHKP');st.write('Munda Living Heritage & Knowledge Project · Munda Cultural Dataset evidence infrastructure.');leadership()
...
st.divider();st.caption('MLHKP · MCD evidence engine · evidence, access and provenance governed through the MLHKP framework.')