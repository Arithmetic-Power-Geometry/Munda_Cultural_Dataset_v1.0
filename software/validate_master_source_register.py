...
def main() -> None:
    ...
    resolver = RefResolver(base_uri=REGISTER_SCHEMA.resolve().as_uri(), referrer=register_schema)
    validator = Draft202012Validator(register_schema, resolver=resolver)
    schema_errors = sorted(validator.iter_errors(register), key=lambda e: list(e.path))
    if schema_errors:
        fail("Register schema validation failed: " + " | ".join(e.message for e in schema_errors[:10]))
    ...
    for row in legacy_rows:
        ...
        if normalize_year(row.get("year")) != normalize_year(rec.get("year")):
            fail(f"{sid}: year changed during migration")
    ...
    print("Universal schema: valid Draft 2020-12")
